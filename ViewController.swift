...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func tagValidation(){
        ...
        if let aValue = UserDefaults.standard.object(forKey: "a") as? [String:Any] {
            ...
            if items.contains(tag) == false {
                ...
                // replace item
                let tagIndex = self.items.firstIndex(of: tag)!
                self.items[tagIndex] = tag
            }
        } else {
            let tag = TagModel(tagId: 0, tagName: "11 Jan, 2019", tagStoreKey: "a")
            tag.save()
            self.collectionView.reloadData()
            print("Default A Value Adding!")
        }
        ...
        if let cValue = UserDefaults.standard.object(forKey: "c") as? [String:Any] {
            ...
            let tag = TagModel(tagId: 2, tagName: "CSKTeam", tagStoreKey: "c")
            tag.save()
            self.collectionView.reloadData()
            print("Default B Value Adding!")
        }
    ...
    // MARK: - Save All Values Action
    
    @IBAction func saveAction(_ sender: Any) {
        
        if UserDefaults.standard.object(forKey: "a") as? [String:Any] == nil {
            print("Select A Value")
        } else if UserDefaults.standard.object(forKey: "b") as? [String:Any] == nil {
            print("Select B Value")
        } else if UserDefaults.standard.object(forKey: "c") as? [String:Any] == nil {
            print("Select C Value")
        } else if UserDefaults.standard.object(forKey: "d") as? [String:Any] == nil {
            print("Select D Value")
        } else if UserDefaults.standard.object(forKey: "e") as? [String:Any] == nil {
            print("Select E Value")
        } else if UserDefaults.standard.object(forKey: "f") as? [String:Any] == nil {
            print("Select F Value")
        } else {
